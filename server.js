...
mongoose.connect('mongodb://localhost:27017/razorpay', { useNewUrlParser: true, useUnifiedTopology: true, useFindAndModify: false });
...
app.post("/:referralCode/:routeKey/orders", function(req, res){
  ...
  User.findOneAndUpdate({ key: req.params.routeKey }, { essay: req.body.essay} , (err, update) => {
    if (err) {
      console.log(err);
    } else {
      console.log("essay updated");
    }
  })
  let options = {
    amount : "50000",
  ...
  User.findOne({key : req.body.routeKey }, function(err, data){
     if(data){
       ...
       let mailDetails = {
           ...
           text: 'Your referral link is http://localhost:3000/' + referralCodeGenerated +" and password is " + data.password + "\nYou can check your balance later using these credentials."
       };
       ...
       var link =  'http://localhost:3000/' + referralCodeGenerated;
